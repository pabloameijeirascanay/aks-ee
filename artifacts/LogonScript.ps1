...
# Set ACR name as a github secret
gh secret set ACR_NAME --body "$Env:acrName"
gh secret set ACR_CLIENT_ID --body "$Env:appId"
gh secret set ACR_CLIENT_PASSWORD --body "$Env:password"
gh secret set ACCESS_TOKEN --body "$Env:GITHUB_TOKEN"

# GitOps
az k8s-configuration flux create `
    -g $Env:resourceGroup `
    -c $Env:arcClusterName `
    --cluster-type connectedClusters `
    --name myconfig `
    --scope namespace `
    --namespace "default" `
    ...
    --branch main `
    --kustomization name=my-kustomization path=./artifacts/k8s-manifests `
    --sync-interval 3s
